...
with col1:
    selected_columns1 = st.multiselect(
        "Select Columns:",
        df.columns,
        default=["gender","SeniorCitizen","Churn"]
    )
...
with col1:
    ...
    feature=st.selectbox("Pick your filter for the pie chart ",selected_columns,key="feature")
with col2:
...
if feature1 != feature2: 
    ...
    fig.update_layout(
    title_font=dict(color='grey'),  # Title color
    title=dict(text=f' {feature1} and  {feature2} ',x=0.27),
    ...
    st.subheader("Columns must be different!!")
    st.subheader("Please select another column")
